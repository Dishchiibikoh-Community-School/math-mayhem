...
// Flat array of all built-in questions
const BUILTIN_QUESTIONS = Object.values(QUESTIONS).flat();

// Load custom questions dynamically
function getCustomQuestionsDB() {
  if (typeof localStorage === 'undefined') return [];
  try {
    return JSON.parse(localStorage.getItem('math_mayhem_custom_questions') || '[]');
  } catch (e) { return []; }
}

function getAllQuestionsCombined() {
  return [...BUILTIN_QUESTIONS, ...getCustomQuestionsDB()];
}

// Utility: get random questions by topic + level
function getQuestions(topic = 'all', level = 1, count = 10) {
  const custom = getCustomQuestionsDB();
  let pool = [];
  ...
  if (topic === 'all') {
    ...
    pool = (QUESTIONS[topic] || []).concat(custom.filter(q => q.topic === topic));
  }
...
function getMayhemQuestions(count = 15) {
  const allQs = getAllQuestionsCombined();
